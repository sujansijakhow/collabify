...
export const submissionRoutes = new Elysia({ prefix: "/submissions" }).post(
  "/",
  async ({ body }) => {
    ...
    // Hand off the heavy lifting (ffmpeg transcode + thumbnail) to the worker
    // via BullMQ so the upload request returns immediately.
